...
if (Test-Path -LiteralPath $statePath -PathType Leaf) {
    try {
        ...
        if ($state.next_scheduled_at) {
            ...
            $scheduledNext = [DateTimeOffset]::Parse($state.last_published_at).ToLocalTime().DateTime.AddHours(24)
        }
...
Register-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description 'Ejecuta Lista M3U en la proxima ventana dinamica de la guia, con limite de 24 horas, y reactiva GitHub Actions el 2 de septiembre de 2026.' `
    -Force | Out-Null

Write-Output "Tarea registrada: $taskName"
Write-Output "Primera ejecucion programada: $($firstRun.ToString('yyyy-MM-dd HH:mm:ss'))"
Write-Output 'La tarea usa una ejecucion unica y se vuelve a programar segun el vencimiento real de la guia, con limite de 24 horas y minimo de 6 horas.'
